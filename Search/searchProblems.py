...
class ClockwiseFoodProblem(SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, self.foods, 'East')
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return state[0] == self.goal and state[1].count() == 0
        util.raiseNotDefined()
    ...
    def getNextStates(self, state):
        ...
        """
        next_states = []
        if self.eat and state[1].count() != 0:
            return []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if (hitsWall):
                continue
            if (Directions.RIGHT[state[2]] != action and state[2] != action):
                continue
            food = state[1].copy()
            ...
            if food.count() == 0:
                self.eat = True
            next_states.append(
                (((nextx, nexty), food, action), action, 1))
            "*** YOUR CODE HERE ***"
        self._expanded += 1  # DO NOT CHANGE
...
class DangerousPositionSearch(PositionSearchProblem):
    ...
    def __init__(self, gameState, goal=(1, 1), start=None, warn=True, visualize=True):
        PositionSearchProblem.__init__(
            self, gameState, self.cost_function, goal, start, warn, visualize)
    ...
    def cost_function(self, state):
        ...
        for i in self.ghosts:
            if (abs(state[0] - i[0]) <= 1 and abs(state[1] - i[1]) <= 1):
                return 999999
        return 1
